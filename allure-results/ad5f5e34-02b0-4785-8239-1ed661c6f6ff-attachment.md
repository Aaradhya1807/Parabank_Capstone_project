...
```ts
  ...
  32 |         await transferPage.enterAmount('50');
  ...
  39 | 
> 40 |         expect(balanceAfter).toBe(balanceBefore - 50);
     |                              ^ Error: expect(received).toBe(expected) // Object.is equality
  41 | 
  42 |         // await page.screenshot({ path: `screenshots/ts-07-test1.png`, fullPage: true });
  43 |     });
  44 | 
  45 | });
  46 | 
